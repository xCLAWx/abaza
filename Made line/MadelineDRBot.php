...
/*
 * طرق مختلفة لتحميل مادلين بروتو
 */
...
{
    const START = "أرسلي لي رابط الملف وسأقوم بتحميله وإرساله لك!\n\n".
                "مثال: `https://example.com`\n".
                "مثال: `https://example.com اسم الملف.ext`\n\n".
                "يمكنني أيضاً إعادة تسمية الملفات فقط أرسل لي أي ملف وسأعيد تسميته!\n\n".
                "الحد الأقصى لتحميل ورفع الملفات هو 1.5 جيجابايت ، مدعومة من قبل : @MadelineArabic.";
    const ADMIN = 'MaGiC_X'; // معرفك/يوزرك

    /**
     * صفوف من محتوى الوسائط
     *
     * @var array
     */
    private $states = [];
    ...
    public function onUpdateNewMessage($update)
    {
        ...
        try {
            ...
            if (isset($update['message']['media']['_']) && $update['message']['media']['_'] !== 'messageMediaWebPage') {
                yield $this->messages->sendMessage(['peer' => $peerId, 'message' => 'أرسل إسم الملف الجديد: ', 'reply_to_msg_id' => $messageId]);
            ...
            $id = yield $this->messages->sendMessage(['peer' => $peerId, 'message' => 'إنتظر قليلاً..', 'reply_to_msg_id' => $messageId]);
            if (!isset($id['id'])) {
            ...
            $url = new \danog\MadelineProto\FileCallback(
                ...
                function ($progress, $speed, $time) use ($peerId, $id) {
                    ...
                    $prev = $now;
                    try {
                        yield $this->messages->editMessage(['peer' => $peerId, 'id' => $id, 'message' =>"نسبة الرفع: $progress%\nالسرعة: $speed ميجابايت/بالثانية\nاستغرق لرفع الملف: $time ثانية"], ['FloodWaitLimit' => 0]);
            ...
            yield $this->messages->sendMedia(
                [
                    ...
                    'message' => 'مدعوم من قبل : @MadelineArabic!',
                    'parse_mode' => 'Markdown'
                ]
            );
            ...
            try {
                yield $this->messages->editMessage(['peer' => $peerId, 'id' => $id, 'message' =>"خطأ: لايمكنني الإتصال بالرابط!\n Error: ".$e->getMessage()]);
            } catch (\Throwable $e) {
                $this->logger((string) $e, \danog\MadelineProto\Logger::FATAL_ERROR);
            }
        }
